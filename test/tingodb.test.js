...
describe('tingodb', function(){
    ...
    it('should allow to find by id string', function (done) {
        Post.create(function (err, post) {
            Post.findById(post.id.toString(), function (err, post) {
                should.not.exist(err);
                should.exist(post);
    ...
    it('findById should return an object with an id, which is instanceof ObjectId', function (done) {
        Post.create(function (err, post) {
    ...
    it('find should return object with an id, which is instanceof ObjectID', function (done) {
        var post = new Post({title: 'a'})
        post.save(function (err, post) {
            Post.find({where: {title: 'a'}}, function (err, posts) {
                should.not.exist(err);
                posts.should.have.lengthOf(1);
                post = posts[0];
                post.id.should.be.an.instanceOf(db.ObjectID);
